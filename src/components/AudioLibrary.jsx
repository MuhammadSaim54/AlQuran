...
export default React.memo(function AudioLibrary({ onPlayRecitation, surahs = [] }) {
  ...
  const currentSelectedSurah = useMemo(() => {
    const found = surahs.find(s => s.number === selectedSurahNumber);
  ...
  // Robust URL generator with 3-digit formatting (001.mp3)
  const resolveAudioUrl = (reciter, surahNum) => {
    if (typeof reciter.getAudioUrl === 'function') {
      return reciter.getAudioUrl(surahNum);
    }
    const padNum = String(surahNum).padStart(3, '0');
    return `https://server${reciter.server || '6'}.mp3quran.net/${reciter.subfolder || reciter.id}/${padNum}.mp3`;
  };

  const handlePlayReciter = (reciter) => {
    const audioUrl = resolveAudioUrl(reciter, currentSelectedSurah.number);
    if (typeof onPlayRecitation === 'function') {
      onPlayRecitation({
  ...
  return (
    <div className="space-y-6 w-full max-w-full">
      <div className="p-5 sm:p-7 rounded-3xl bg-white/85 border border-gold/30 shadow-md space-y-4 relative z-30">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  <ChevronDown className={`w-4 h-4 text-earth-muted transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`} />
                </div>
              </button>
                  ...
                  >
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-2 rounded-xl bg-cream/70 border border-gold/25 text-xs outline-none focus:border-gold text-earth-text"
                        autoFocus
                      />
                    </div>
                    ...
                    <div className="max-h-56 overflow-y-auto space-y-1 pr-1">
                      {filteredSurahsList.length === 0 ? (
                        <p className="text-center text-xs text-earth-muted py-3">No Surah found</p>
                      ) : (
            ...
            <div className="relative w-full sm:w-60">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-white border border-gold/30 text-xs sm:text-sm outline-none focus:border-gold shadow-sm h-11 text-earth-text"
                />
              </div>
            </div>
          </div>
